...
-------------------------------------------------------------------------------
  ...
  $Header: //components/rel/boot.xf/3.2/QcomPkg/Library/ICBArbLib/ul_arb.c#1 $
...
)
{
   ...
   bool is_complete_request = false, is_active = false, remove_resource_active = false;
    ...
    * We only count REQUIRED vector clients. */
   if( NPA_CLIENT_VECTOR == client->type )
   {
      ...
      if( prev_active && !is_active )
      {
         ...
         if( 0 < resource_data->num_active_required_clients )
         {
            ...
            {
               remove_resource_active = true;
            }
         }
      }
      /* If not previously active, but now active, incremente count.*/
   ...
   ul_request_commit( resource_data->topology, client->request_attr );

   /* Remove active indication if necessary. */
   if( remove_resource_active )
   {
      ul_remove_active();
   }
