...
boolean plat_clock_enable (plat_device_config *config)
{
	...
	clock_index = (config->qupv3_instance*MAX_SE) + (config->se_index + 1);  // 1 added to counter for CLK_QUPV3_NONE in ClockQUPV3Type
	
	if(Clock_SetQUPV3Frequency((ClockQUPV3Type)clock_index, 19200000) == FALSE)
	{
        dalRes = FALSE;
	    goto ON_EXIT;
    }
...
boolean plat_clock_disable (plat_device_config *config)
{
	...
	if(Clock_DisableQUPV3Clock((ClockQUPV3Type)clock_index) == FALSE)
	{
       dalRes = FALSE;
	   goto ON_EXIT;
	}
