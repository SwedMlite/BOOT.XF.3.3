/*                                                                    
  ...
  Copyright (c) 2015,2018 Copyright Qualcomm Technologies, Inc.  All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 12/24/18   pr       Changes to read Debug Policy Flag
...
)
{
  ...
  *InformationBlock = AllocatePool(sizeof(ADAPTER_INFO_PLATFORM_SECURITY));
  ...
  debugpolicyflag = GetDebugPolicyFlag();              // FALSE implies secure device
  if(!debugpolicyflag)
  {
...
BOOLEAN GetDebugPolicyFlag(VOID)
{
  ...
  sec_dbg_t*                  ptr_sec_dbg_t = (sec_dbg_t*)UncachedAllocateAlignedPool(sizeof(sec_dbg_t), TZ_MEM_ALIGNMENT_SIZE);
