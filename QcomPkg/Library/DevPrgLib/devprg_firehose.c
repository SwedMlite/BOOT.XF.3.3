...
/*===========================================================================
  ...
  $Header: //components/rel/boot.xf/3.2/QcomPkg/Library/DevPrgLib/devprg_firehose.c#6 $
...
2019-08-14   jt    Add support to get JEDEC-compliant turbo write parameters 
...
static int get_dev_data(struct dp_xml_handle *handle, struct dp_basic_dev_data *io)
{
  ...
  if (res != 0)
  {
    if (io->storage_type == DEVPRG_STORAGE_SDCC)
      io->slot = 1;
    else
...
int handle_poke(struct dp_xml_handle *xml_handle)
{
  ...
  /* If secure boot is enabled, disable poke. */
  if (devprg_is_secure_boot_enabled() != 0)
  {
    DP_LOGW("Poke is disabled on secure boot devices%c", '.');
    goto error;
  }
