...
/*===========================================================================
  ...
  $Header: //components/rel/boot.xf/3.2/QcomPkg/Library/DevPrgLib/devprg_target.c#2 $
  $DateTime: 2020/06/26 15:06:06 $
  $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2020-06-25   cm   Invalidate indirect_arg_list buffers .
...
static sec_img_auth_error_type boot_sec_img_auth_verify_signature(uint8 *buf, uint32 size, uint32 sw_id, uint32 sw_ver, uint32 code_segment, uint8 **data_address, uint32 *data_size)
{
  ...
  memset(&v_info_inst, 0, sizeof(v_info_inst));
  ...
  {
    dcache_inval_region(&v_info_inst, sizeof(v_info_inst));
