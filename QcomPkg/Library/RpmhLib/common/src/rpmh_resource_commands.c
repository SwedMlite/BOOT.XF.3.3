...
//      {
      //simplify for UEFI - if there is a valid active vote, we should send a wake vote because
      //UEFI sleep may put in the sleep vote
      else if ((!wake_vote->valid) && 
               ((active_vote->valid) && 
                //prevent dynamic VRMs, but allow BCM and ARC
                ((self->address >> 16) != 4) &&
                (sleep_vote->dirty != RPMH_RC_CLEAN_EXPLICIT_VOTE)))
      {
        //need to send a wake vote
