  ...
  )
{
  ...
  UINTN                                CapsuleFileNameStrSize = 0;
  ...
  if (CapsuleFileName != NULL) {
    CapsuleFileNameStrSize = StrSize(CapsuleFileName);
  } else {
    CapsuleFileNameStrSize = sizeof(CHAR16);
  }

  CapsuleResultVariableSize = sizeof(EFI_CAPSULE_RESULT_VARIABLE_HEADER) + sizeof(EFI_CAPSULE_RESULT_VARIABLE_FMP) + CapsuleFileNameStrSize + DevicePathStrSize;
  ...
  if (CapsuleFileName != NULL) {
    CopyMem ((UINT8 *)CapsuleResultVariableFmp + sizeof(EFI_CAPSULE_RESULT_VARIABLE_FMP), CapsuleFileName, CapsuleFileNameStrSize);
  }

  if (DevicePathStr != NULL) {
    CopyMem ((UINT8 *)CapsuleResultVariableFmp + sizeof(EFI_CAPSULE_RESULT_VARIABLE_FMP) + CapsuleFileNameStrSize, DevicePathStr, DevicePathStrSize);
