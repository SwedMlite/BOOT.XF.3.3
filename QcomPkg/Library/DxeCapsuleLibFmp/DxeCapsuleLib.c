...
/**
  ...
  @param[in]   CapsuleFileName       In case of capsule loaded from disk, the zero-
                                     terminated array containing file name of capsule
                                     that was processed
  @param[out]  ResetNeeded           Returns if a process capsule that needs a reset was
                                     processed
  ...
  )
{
  ...
  BOOLEAN                                       NotReady;
  BOOLEAN                                       Abort;
  BOOLEAN                                       ResetAttributeSet;

  if (!IsFmpCapsuleGuid(&CapsuleHeader->CapsuleGuid)) {
    return ProcessFmpCapsuleImage ((EFI_CAPSULE_HEADER *)((UINTN)CapsuleHeader + CapsuleHeader->HeaderSize), CapsuleFileName, ResetNeeded);
  }
  ...
  for (Index = FmpCapsuleHeader->EmbeddedDriverCount; Index < ItemNum; Index++) {
    ...
    for (Index2 = 0; Index2 < NumberOfHandles; Index2++) {
      ...
      if (Status != EFI_SUCCESS) {
        DEBUG ((DEBUG_WARN, "Failed to set the firmware payload %d. Status = %r\n", Index, Status));
        Abort = TRUE;
      }
      ...
      if (NULL != ResetNeeded) {
        if (EFI_SUCCESS == GetFmpImageInfoImageAttribResetRequired(HandleBuffer[Index2],
                                                                   &ResetAttributeSet)) {
          *ResetNeeded = ResetAttributeSet;
        }                                             
      }
    }
...
/**
  ...
  @param[in]  CapsuleHeader    Point to the UEFI capsule image to be processed. 
  ...
  )
{
  ...
  if (IsFmpCapsule (CapsuleHeader)) {
    ...
    Status = ProcessFmpCapsuleImage(CapsuleHeader, CapsuleFileName, ResetNeeded);
