...
$Header: //components/rel/boot.xf/3.2/QcomPkg/Library/PmicLib/drivers/fg/src/pm_fg_batt_soc.c#3 $
$Author: pwbldsvc $
...
pm_err_flag_type pm_fg_batt_soc_get_monotonic_soc_cp(uint32 device_index, uint32* soc, boolean* soc_match_cp)
{
  ...
  else
  {
    ...
    err_flag = pm_comm_read_byte_array(SLAVE_ID, monotonic_soc, 2, (uint8*) &data);
...
pm_err_flag_type pm_fg_batt_soc_enable_fg_algorithm(uint32 device_index, boolean enable)
{
   ...
   else
   {
      ...
      err_flag = pm_comm_write_byte_mask(SLAVE_ID, en_ctl, 0x80, data);
   }
...
pm_err_flag_type pm_fg_batt_soc_restart(uint32 device_index, boolean restart)
{
   ...
   else
   {
      ...
      err_flag = pm_comm_write_byte_mask(SLAVE_ID, restart, 0x1, data);
   }
...
pm_err_flag_type pm_fg_get_batt_soc_enable_ctl(uint32 pmic_device, boolean *enable)
{
  ...
  else
  {
    ...
    err_flag = pm_comm_read_byte(SLAVE_ID, fg_batt_soc_en_ctl_reg, &data);
