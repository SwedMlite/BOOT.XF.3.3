...
$Header: //components/rel/boot.xf/3.2/QcomPkg/Library/PmicLib/drivers/clk/src/pm_clk.c#4 $
$Author: pwbldsvc $
...
#define REG_CLK(reg) ((clk_ptr->clk_reg_table->base_address) + \
...
pm_err_flag_type pm_clk_drv_strength(uint32 pmic_chip, pm_clk_type periph, pm_clk_drv_strength_type drive_strength)
{
    ...
    /* Set strength */
    err_flag = pm_comm_write_byte_mask(clk_ptr->comm_ptr->slave_id, reg, 0x03, (pm_register_data_type)(drive_strength));  
