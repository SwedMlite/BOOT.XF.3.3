...
$Header: //components/rel/boot.xf/3.2/QcomPkg/Library/PmicLib/drivers/clk/src/pm_clk_sleep.c#4 $
$Author: pwbldsvc $
...
pm_err_flag_type pm_clk_sleep_cal_rc_enable(uint8 pmic_chip, pm_clk_type periph, pm_on_off_type on_off)
{
    ...
    err_flag = pm_comm_write_byte_mask(clk_ptr->comm_ptr->slave_id, reg, 0x80, data);
...
pm_err_flag_type pm_clk_sleep_coincell_valid(uint8 pmic_chip, pm_clk_type periph, pm_on_off_type on_off)
{
    ...
    err_flag = pm_comm_write_byte_mask(clk_ptr->comm_ptr->slave_id, reg, 0x40, data);
