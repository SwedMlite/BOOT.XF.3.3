...
$Header: //components/rel/boot.xf/3.2/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs_info.c#5 $
$Author: pwbldsvc $
...
static pm_err_flag_type pm_pbs_info_otp_read (uint32 slave_id, pm_pbs_info_smem_type *pbs_info_ptr)
{
  ...
  err_flag = pm_comm_read_byte_array(slave_id, PMIO_PBS_CORE_MEM_INTF_RD_DATA0_ADDR, 4, (uint8*)&read_info_temp);
...
static pm_err_flag_type pm_pbs_info_read_from_periph (uint32 slave_id, pm_pbs_info_smem_type *pbs_info_ptr)
{
  ...
  err_flag = pm_comm_read_byte_array(slave_id, PMIO_PBS_CORE_MEM_INTF_RD_DATA0_ADDR, 4, (uint8*)&read_info_temp32);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    return err_flag;
  }
...
pm_err_flag_type pm_pbs_info_rom_init (void) 
{
  ...
  for (pmic_index = 0; pmic_index < PM_MAX_NUM_PMICS; pmic_index++)
  {
    ...
    err_flag = pm_comm_read_byte(slave_id, PMIO_PBS_CORE_PERPH_TYPE_ADDR, &data);
    ...
    else if(pbs_header_version == PM_PBS_HEADER_VER_2)
{
    ...
    */
    ...
    if(pon_reason_dataset_index !=  PBS_DATASET_INVALID_INDEX)
    {
        ...
        {
          pbs_header_dataset_ptr->pon_reason_sdam_info= ((read_data[pon_reason_dataset_index] & PM_PBS_PON_RSN_SDAM_ADRESS_MASK)) ;

          if(((pbs_header_dataset_ptr->pon_reason_sdam_info >> 24) & 0x000000FF) == 0x00)
          {
            pbs_header_dataset_ptr->pon_reason_dataset_address = ((read_data[pon_reason_dataset_index] & PM_PBS_DATASET_START_ADDRESS_MASK)) ;
          }
          else
          {
...
pm_err_flag_type pm_pbs_ram_dataset_read(uint32 pmic_index, uint32* dataset_ptr, uint32 dataset_ptr_size, pm_pbs_header_dataset_type dataset_type)
{
	...
	switch(dataset_type)
    {
      ...
      case PM_PBS_HEADER_PON_REASON_DATASET:
           ...
           if(( (pm_pbs_header_dataset_arr[pmic_index].pon_reason_sdam_info >> 24) & 0x000000FF) == 0x00)
           {
             dataset_size  =  (pm_pbs_header_dataset_arr[pmic_index].pon_reason_sdam_info >> 16) & 0x000000FF;
             dataset_size  = (uint32) (dataset_size)  / 4;
           }
           else
           {
    ...
    {
      //if PON RSNs are stored in SDAM , byte 3,2 will be 0x00.
      if(dataset_type == PM_PBS_HEADER_PON_REASON_DATASET && (( (pm_pbs_header_dataset_arr[pmic_index].pon_reason_sdam_info >> 24) & 0x000000FF) == 0x00))
      {
        pm_pbs_pon_rsn_sdam_read(slave_id, start_address, dataset_ptr, dataset_size);
      }
      else
      {
        /* Retrieve all the pon data information from PBS ram */
        err_flag |= pm_get_slave_id(pmic_index, 0, &slave_id);
        err_flag |= pm_pbs_mem_read(slave_id, pm_pbs_data->pbs_ram_start_addr + start_address, dataset_ptr, dataset_size);
      }
    }
...
pm_err_flag_type pm_pbs_ram_dataset_write(uint32 pmic_index, uint32 *dataset_ptr, uint32 dataset_ptr_size, pm_pbs_header_dataset_type dataset_type)
{
    ...
    err_flag |= pm_get_slave_id(pmic_index, 0, &slave_id);
    err_flag |= pm_pbs_mem_write(slave_id, pm_pbs_data->pbs_ram_start_addr + start_address, (uint32 *)local_dataset_ptr, data_size);
...
pm_err_flag_type pm_pbs_mem_read(uint8 slave_id, uint32 start_address, uint32* data_ptr, uint32 data_size)
{
    ...
    {
       	err_flag |= pm_comm_read_byte_array(slave_id, PMIO_PBS_CORE_MEM_INTF_RD_DATA0_ADDR, 4, (uint8 *)&read_data);
       	if (err_flag != PM_ERR_FLAG_SUCCESS)
       	{
          	err_flag |= pm_pbs_disable_access(slave_id, PM_PBS_TYPE_RAM);
    		return err_flag;
...
pm_err_flag_type pm_pbs_mem_write(uint8 slave_id, uint32 start_address, uint32* data_ptr, uint32 data_size)
{
    ...
    for (i =0 ; i < data_size ; i++)
    {
       err_flag |= pm_comm_write_byte_array(slave_id, PMIO_PBS_CORE_MEM_INTF_WR_DATA0_ADDR, 4, (uint8*)&data_ptr[i]);
        if (err_flag != PM_ERR_FLAG_SUCCESS)
        {
          err_flag |= pm_pbs_disable_access(slave_id, PM_PBS_TYPE_RAM);
          return err_flag;
