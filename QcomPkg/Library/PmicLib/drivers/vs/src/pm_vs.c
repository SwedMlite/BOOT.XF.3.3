...
$Header: //components/rel/boot.xf/3.2/QcomPkg/Library/PmicLib/drivers/vs/src/pm_vs.c#3 $
$Author: pwbldsvc $
...
pm_vs_sw_mode(uint8 pmic_chip, uint8 vs_peripheral_index, pm_sw_mode_type mode) 
{
    ...
    else
    {
        ...
        {
            err_flag = pm_comm_write_byte_mask(vs_ptr->comm_ptr->slave_id, reg, mask, data);
        }
    }
...
pm_err_flag_type 
pm_vs_sw_enable(uint8 pmic_chip, uint8 vs_peripheral_index, pm_on_off_type on_off, boolean wait_for_settle)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_vs_data_type *vs_ptr = pm_vs_get_data(pmic_chip);
    ...
    { 
        err_flag = pm_pwr_sw_enable_alg(&(vs_ptr->pm_pwr_data), vs_ptr->comm_ptr, vs_peripheral_index, on_off, wait_for_settle);
    }
