...
$Header: //components/rel/boot.xf/3.2/QcomPkg/Library/PmicLib/drivers/pon/src/pm_pon.c#7 $
$Author: pwbldsvc $
$DateTime: 2019/08/22 17:12:25 $
...
pm_pon_init(void)
{
  ...
  err_flag |= pm_comm_read_byte(PMIC_A_SLAVEID_PRIM, PMIO_PON_PERPH_TYPE_ADDR, &periph_type);
  ...
  err_flag |= pm_comm_read_byte(PMIC_A_SLAVEID_PRIM, PMIO_PON_GEN2_ON_REASON_ADDR, warm_reset_reason);
  if (warm_reset_reason[0] & PMIO_PON_GEN2_ON_REASON_WARM_SEQ_BMSK)
  {
    err_flag |= pm_comm_write_byte_mask(PMIC_A_SLAVEID_PRIM, PMIO_PON_DVDD_RB_SPARE_ADDR, PON_DVDD_RB_SPARE_WARM_RESET_MASK, 0x01);
  }
                        ...
                        uint32 s1_timer, uint32 s2_timer, pm_pon_reset_cfg_type reset_cfg)
{
  ...
  {
    err_flag |= pm_comm_write_byte_mask(slave_id, pon_trigger_en_reg.ctl_addr, pon_trigger_en_reg.kpwpwr_mask, 0x0);
  ...
  if (reset_source == PM_PON_RESET_SOURCE_KPDPWR)
  {
    err_flag |= pm_comm_write_byte_mask(slave_id, pon_trigger_en_reg.ctl_addr, pon_trigger_en_reg.kpwpwr_mask, 0xFF);
    WAIT_FOR_SLEEP_CLK_CYCLE(3);
  }
...
pm_pon_stage3_reset_source_cfg(uint32 pmic_device_index, pm_pon_reset_source_type reset_source, uint32 s3_timer)
{
  ...
  err_flag = pm_comm_write_byte_mask(slave_id, PMIO_PON_RESET_S3_SRC_ADDR, PMIO_PON_RESET_S3_SRC_RMSK, reset_source);
  WAIT_FOR_SLEEP_CLK_CYCLE(5);
  ...
  err_flag |= pm_comm_write_byte_mask(slave_id, PMIO_PON_RESET_S3_TIMER_ADDR, PMIO_PON_RESET_S3_TIMER_RMSK, s3_timer_id);
  WAIT_FOR_SLEEP_CLK_CYCLE(5);
...
pm_pon_get_fsm_current_state(uint8 pmic_device_index, pm_pon_fsm_status_type *pon_fsm_status)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte(slave_id, PMIO_PON_GEN2_FSM_STATUS_ADDR, &fsm_status_val);
...
pm_pon_get_pon_reason(uint32 pmic_device_index, pm_pon_reason_type *reason)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte(slave_id, PMIO_PON_GEN2_PON_REASON1_ADDR, (uint8 *)reason);
    }
  }
...
pm_pon_get_poff_reason(uint32 pmic_device_index, pm_pon_poff_reason_type *reason)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte_array(slave_id, PMIO_PON_GEN2_POFF_REASON1_ADDR, 2, (uint8 *)reason);
    }
  }
...
pm_pon_get_warm_reset_reason(uint32 pmic_device_index, pm_pon_warm_reset_reason_type *reason)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte_array(slave_id, PMIO_PON_GEN2_WARM_RESET_REASON1_ADDR, 1, (uint8 *)reason);
    }
  }
...
pm_pon_get_on_reason(uint32 pmic_device_index, pm_pon_on_state_reason_type *reason)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte(slave_id, PMIO_PON_GEN2_ON_REASON_ADDR, (uint8 *)reason);
    }
  }
...
pm_pon_get_off_reason(uint32 pmic_device_index, pm_pon_off_state_reason_type *reason)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte(slave_id, PMIO_PON_GEN2_OFF_REASON_ADDR, (uint8*)reason);
    }
  }
...
pm_pon_get_stage3_reset_reason(uint32 pmic_device_index, pm_pon_stage3_reset_reason_type *reason)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte(slave_id, PMIO_PON_GEN2_S3_RESET_REASON_ADDR, (uint8 *)reason);
    }
  }
...
pm_pon_get_all_pon_reasons(uint32 pmic_device_index, uint64 *pon_reasons)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte_array(slave_id, PMIO_PON_GEN2_PON_REASON1_ADDR, 8, (uint8 *)pon_reasons);
    }
  }
...
pm_pon_get_fault_reason(uint32 pmic_device_index, pm_pon_fault_state_reason_type *reason)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte_array(slave_id, PMIO_PON_GEN2_FAULT_REASON1_ADDR, 2, (uint8*)reason);
    }
  }
...
pm_pon_get_fault_reasons(uint32 pmic_device_index, uint64 *fault_reasons)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte_array(slave_id, PMIO_PON_GEN2_FAULT_REASON1_ADDR, 4, (uint8 *)fault_reasons);
    }
  }
...
pm_pon_debounce_ctl(uint32 pmic_device_index, uint32 time_delay)
{
  ...
  if (err_flag == PM_ERR_FLAG_SUCCESS)
  {
    err_flag = pm_comm_write_byte_mask(slave_id, PMIO_PON_DEBOUNCE_CTL_ADDR,
                                       PMIO_PON_GEN2_DEBOUNCE_CTL_DEBOUNCE_BMSK, time_id);
...
pm_pon_irq_clear(uint32  pmic_device_index, pm_pon_irq_type irq)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_write_byte(slave_id, PMIO_PON_INT_LATCHED_CLR_ADDR, mask);
    }
  }
...
pm_pon_irq_set_trigger(uint32 pmic_device_index, pm_pon_irq_type irq, pm_irq_trigger_type trigger)
{
  ...
  if (err_flag == PM_ERR_FLAG_SUCCESS)
  {
    err_flag = pm_comm_write_byte_mask(slave_id, PMIO_PON_INT_SET_TYPE_ADDR, mask, set_type);
    err_flag |= pm_comm_write_byte_mask(slave_id, PMIO_PON_INT_POLARITY_HIGH_ADDR, mask, polarity_high);
    err_flag |= pm_comm_write_byte_mask(slave_id, PMIO_PON_INT_POLARITY_LOW_ADDR, mask, polarity_low);
  }
...
pm_pon_config_uvlo(uint32 pmic_device_index, uint32 uvlo_thresh_voltage_in_mv, uint32 uvlo_hyst_voltage_in_mv, boolean uvlo_enable)
{
  ...
  {
    //Config UVLO CTL
    err_flag |= pm_comm_write_byte(slave_id, PMIO_PON_UVLO_CTL, uvlo_ctl_data);
    WAIT_FOR_SLEEP_CLK_CYCLE(3);
  }
...
pm_err_flag_type pm_pon_smpl_enable(uint32 pmic_device_index, boolean enable)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint32 slave_id = 0;
  uint8 data = enable ? PMIO_PON_GEN2_SMPL_EN_BMSK : 0;

  err_flag = pm_pon_get_pon_info(pmic_device_index, &slave_id);
  if(PM_ERR_FLAG_SUCCESS == err_flag)
  {
    err_flag = pm_comm_write_byte_mask(slave_id, PMIO_PON_GEN2_SMPL_CTL_ADDR, PMIO_PON_GEN2_SMPL_EN_BMSK, data);
  }
...
pm_pon_warm_reset_status(uint32 pmic_device_index, boolean *status)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      ...
      err_flag = pm_comm_read_byte(slave_id, PMIO_PON_DVDD_RB_SPARE_ADDR, &data);
...
pm_pon_warm_reset_status_clear(uint32 pmic_device_index)
{
  ...
  if (err_flag == PM_ERR_FLAG_SUCCESS)
  {
    err_flag = pm_comm_write_byte_mask(slave_id, PMIO_PON_DVDD_RB_SPARE_ADDR, PON_DVDD_RB_SPARE_WARM_RESET_MASK, 0x00);
  }
...
pm_pon_pbl_get_status(uint32  pmic_device_index, pm_pon_pbl_status_type pbl_status, boolean *status)
{
  ...
  err_flag = pm_comm_read_byte(slave_id, PMIO_PON_PON_PBL_STATUS_ADDR, &data);
...
pm_pon_trigger_enable_status(uint32 pmic_device_index, pm_pon_trigger_type trigger, boolean *status)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      err_flag = pm_comm_read_byte(slave_id, PMIO_PON_PON_TRIGGER_EN_ADDR, &pon_trigger);
...
pm_pon_trigger_enable(uint32 pmic_device_index, pm_pon_trigger_type trigger, boolean enable)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      ...
      err_flag = pm_comm_write_byte_mask(slave_id, PMIO_PON_PON_TRIGGER_EN_ADDR, mask, data);
...
pm_err_flag_type pm_pon_set_option_bit(uint32 pmic_device_index, pm_pon_option_bit_type cfg_field, boolean cfg_data)
{
  ...
  else
  {
    ...
    if (err_flag == PM_ERR_FLAG_SUCCESS)
    {
      ...
      err_flag |= pm_comm_write_byte_mask(slave_id, PMIO_PON_OPTION_BIT_ADDR, mask, write_data);
      //Read the data from 0x882 and Write it to 0x881
      err_flag |= pm_comm_read_byte(slave_id, PMIO_PON_OPTION_BIT_ADDR, &read_data);
      err_flag |= pm_comm_write_byte(slave_id, PMIO_PON_TRIGGER_EDGE_CONFIG, read_data);
    }
  }
