...
/*======================================================================
  ...
  $Header: //components/rel/boot.xf/3.2/QcomPkg/Library/UfsCommonLib/ufs_utp_core.c#10 $
  $DateTime: 2020/06/17 10:46:30 $ $Author: pwbldsvc $

when         who     what, where, why
----------   ---     --------------------------------------------------- 
2020-01-06   jt      Use symmetric lane configuration 
...
int32_t ufs_utp_poll_pms_complete (ufs_host_t *hba)
{
   uint32_t tout = UFS_HCI_UPMS_POLL_TIMEOUT;
   uint32_t val;
...
int32_t ufs_utp_gear_sw (ufs_host_t *hba)
{
   ...
   rc = ufs_utp_uic_getset (hba, UICCMDR_DME_SET, MIBattr(PA_PWRModeUserData(3), 0), 0x1FFF, NULL);
   if (rc != EOK) {
      ufs_error_log(hba, 0x003c);
      return rc;
   }

   rc = ufs_utp_uic_getset (hba, UICCMDR_DME_SET, MIBattr(PA_PWRModeUserData(4), 0), 0xFFFF, NULL);
   if (rc != EOK) {
      ufs_error_log(hba, 0x003d);
      return rc;
   }

   rc = ufs_utp_uic_getset (hba, UICCMDR_DME_SET, MIBattr(PA_PWRModeUserData(5), 0), 0x7FFF, NULL);
   if (rc != EOK) {
...
int32_t ufs_utp_link_h8 (ufs_host_t *hba, uint8 enter)
{
   int32_t rc = EOK;
   uint32_t tout, val; 
   uint32 bmsk; 
   ...
   int errcnt = UFS_MAX_ERR_CNT;
      ...
      if ( HWIO_REG_IN (hba->ufs_hci_addr, UFSHCI_IS) & BMSK_UTRCS) {
         if (! (HWIO_REG_IN (hba->ufs_hci_addr, UFSHCI_UTRLDBR) & 
            ...
            // in some rare cases
            ufs_bsp_cache_op ((void *)(uintptr_t)pinfo->utp_cmd_desc_phy, 
                              pinfo->prdt_offset + (hba->prdtCount * PRDT_SIZE), CLEAN);
...
int32_t ufs_utp_init (ufs_host_t *hba)
{
   ...
   // Link startup + PHY initialization can be skipped for primary UFS  
   if ((hba->plat_cfg.core_skip) && (hba->ufs_core_id == 0))
   {
...
{
   boolean rateb = FALSE, gear4 = FALSE; 
   uint16_t flags = 0, offs = 0; 
   uint32_t i = 0; 
   uint8_t val;

   rateb = (UFS_DEFAULT_HS_SERIES == 0x02) ? TRUE : FALSE; 
   gear4 = ((hba->dal_cfg.enable_hs) && (hba->dal_cfg.gear >= 4)) ? TRUE : FALSE; 
