...
$Header: //components/rel/boot.xf/3.2/QcomPkg/Library/ICBLib/icbcfg.c#2 $ 
$DateTime: 2019/05/03 15:17:29 $
$Author: pwbldsvc $
$Change: 19086985 $ 
        ...
        channel++ )
   {
      ...
      while( map_idx < MAX_REGIONS && hw_idx < MAX_REGIONS )
      {
         ...
         switch(match)
         {
            ...
            {
               icbcfg_mem_region_type *map_ddr_region;
               uint64 size = remap->size;
               next_start_addr = curr_region->base_addr + remap->size;

               /* Make sure we don't exceed the valid range for the end of this region. */
