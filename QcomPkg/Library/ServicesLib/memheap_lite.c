...
$Header: //components/rel/boot.xf/3.2/QcomPkg/Library/ServicesLib/memheap_lite.c#3 $ $DateTime: 2019/06/05 18:52:32 $ $Author: pwbldsvc $
...
)
{
  ...
  if ( (!pAlloc) || // Fail.
       (!((unsigned int)pAlloc & alignMask)) ) // Success & aligned.
  {
    END_CRITICAL_SECTION(heap_ptr);
    return pAlloc;
  }
