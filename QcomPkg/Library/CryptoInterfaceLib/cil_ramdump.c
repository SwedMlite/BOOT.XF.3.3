...
static uint8  ALIGN(32)           dload_aes_key[XBL_SW_AES_KEY_SIZE];
static uint8  ALIGN(32)           dload_hmac_key[XBL_SW_HMAC_KEY_SIZE];
static uint8  ALIGN(32)           dload_minidump_version[XBL_MINIDUMP_VERSION_SIZE] = {0x00,0x01};// format is MSB : LSB -> Major Version : Minor Version
static uint8  ALIGN(32)           dload_aes_and_hmac_key_and_version[XBL_SW_AES_PLUS_HMAC_KEY_PLUS_VERSION_SIZE];
static uint8  ALIGN(32)           dload_rsa_encr_key[XBL_RSA_KEY_LEN];

static uint8  ALIGN(32)           dload_aes_key_iv[NUM_REGIONS][XBL_SW_AES_IV_SIZE];
static uint8  ALIGN(32)           dload_aes_key_mac[NUM_REGIONS][XBL_SW_AES_MAC_SIZE];
