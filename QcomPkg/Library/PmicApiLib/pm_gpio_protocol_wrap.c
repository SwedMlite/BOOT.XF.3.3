...
pm_err_flag_type pm_gpio_enable(uint32 pmic_chip, pm_gpio_perph_index gpio, boolean  enable)
{
  pm_err_flag_type       err_flag = PM_ERR_FLAG_SUCCESS;
  EFI_STATUS  Status = EFI_SUCCESS;
  ...
  if (PM_ERR_FLAG_SUCCESS == pm_gpio_protocol_init())
  {
    ...
    Status = PmicGpioProtocol->Enable(pmic_chip, (EFI_PM_GPIO_PERIPH_INDEX)gpio, enable);
