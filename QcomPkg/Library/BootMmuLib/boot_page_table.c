...
Copyright 2012-2015,2017-2019 by Qualcomm Technologies, Inc.  All Rights Reserved.
=============================================================================*/
...
--------   ---     ------------------------------------------------------------
03/28/19   yps     Added padd/vadd/size alignment check
03/22/18   yps     Propagate attributes from L1/L2 to subsequent L2/L3 pagetables
...
)
{
  ...
  address_base_index = (block->v_base & L1_PT_INDEX) >> GB_TO_BYTES_SHIFT;

  /* only map 1GB size sections ,  round up to 1GB alignment*/
  limit = address_base_index + (((block->size_in_bytes >> KB_TO_BYTES_SHIFT) + 0xFFFFF) >> MB_TO_BYTES_SHIFT);
...
)
{
  ...
  */
  ...
  do
  {
    ...
    limit = address_base_index + (((block->size_in_bytes >> KB_TO_BYTES_SHIFT) + 2047) >> 11);
...
)
{
  ...
  */
  ...
  do
  {
    ...
    limit = address_base_index + (((block->size_in_bytes >> KB_TO_BYTES_SHIFT) + 3) >> 2);
...
)
{
  ...
  for (map_ptr = map; map_ptr->v_base != MMU_LAST_ADDRESS; map_ptr++)
  {
    ...
    {
     /* Checking address and size align with L1 page size*/
      BL_VERIFY(!(map_ptr->v_base%MMU_L1_SECTION_MAPPING_PAGE_SIZE), BL_ERR_MMU_INVALID_ADDRESS | BL_ERROR_GROUP_BOOT);
	  BL_VERIFY(!(map_ptr->size_in_bytes%MMU_L1_SECTION_MAPPING_PAGE_SIZE), BL_ERR_MMU_INVALID_SIZE | BL_ERROR_GROUP_BOOT);
    ...
    {
      /* Checking address and size align with L2 page size*/
      BL_VERIFY(!(map_ptr->v_base%MMU_L2_SECTION_MAPPING_PAGE_SIZE), BL_ERR_MMU_INVALID_ADDRESS | BL_ERROR_GROUP_BOOT);
	  BL_VERIFY(!(map_ptr->size_in_bytes%MMU_L2_SECTION_MAPPING_PAGE_SIZE), BL_ERR_MMU_INVALID_SIZE | BL_ERROR_GROUP_BOOT);
    ...
    {
      /* Checking address and size align with L3 page size*/
      BL_VERIFY(!(map_ptr->v_base%MMU_L3_SECTION_MAPPING_PAGE_SIZE), BL_ERR_MMU_INVALID_ADDRESS | BL_ERROR_GROUP_BOOT);
	  BL_VERIFY(!(map_ptr->size_in_bytes%MMU_L3_SECTION_MAPPING_PAGE_SIZE), BL_ERR_MMU_INVALID_SIZE | BL_ERROR_GROUP_BOOT);
      ...
      end_address = map_ptr->p_base + map_ptr->size_in_bytes;     
      ...
      if( end_address > l2_block_end )
      {
        ...
        temp_table.size_in_bytes = l2_block_end - map_ptr->p_base;
        mmu_map_block_as_l3_pages(mmu_l1_pagetable, mmu_l2_pagetable, &temp_table);
        temp_table.p_base += temp_table.size_in_bytes;
        temp_table.v_base += temp_table.size_in_bytes;
        /* boundary to end address */
        temp_table.size_in_bytes = map_ptr->size_in_bytes - temp_table.size_in_bytes;
        mmu_map_block_as_l3_pages(mmu_l1_pagetable, mmu_l2_pagetable, &temp_table);
      }
      else
      {
    ...
    {
      result = FALSE;
      /* Assert instead of silently failing */
      BL_VERIFY(result == TRUE, BL_ERR_MMU_PGTBL_MAPPING_FAIL | BL_ERROR_GROUP_BOOT);
    }
  }
...
)
{
  ...
  uint64 address_base_index;
  address_base_index = (map_ptr->v_base & L1_PT_INDEX) >> GB_TO_BYTES_SHIFT; 
  ...
  for( i = 0; i < sizeof(sbl_imem_ranges)/sizeof(xbl_internal_mem_region_type); i++)
  {
    if(entry_invalid_enable == TRUE)
    {
      if((map_ptr->v_base >= sbl_imem_ranges[i].start_addr && map_ptr->v_base < sbl_imem_ranges[i].start_addr + sbl_imem_ranges[i].size)
	  	&&(map_ptr->v_base + map_ptr->size_in_bytes) < (map_ptr->v_base < (sbl_imem_ranges[i].start_addr + sbl_imem_ranges[i].size)))
      {
  ...
  if (entry_invalid_enable == TRUE)
  {
    ...
    boot_mmu_find_entry_and_unmap((uintnt*)mmu_l1_pagetable,(uintnt )map_ptr->v_base,(uintnt )map_ptr->size_in_bytes);
  }
  ...
  do
  {
            ...
            map_ptr->memory_mapping == MMU_L3_NS_SECTION_MAPPING)
    {
	     ...
	     (empty_l3_pt_needed <= empty_l3_pagetable_num))
      {
        /* Checking for block overflow, if an L3 block of pages overlaps from one 
           L2 block to another L2 block then L3 block must split and added as 2 entries */
        end_address = map_ptr->p_base + ((uint32)map_ptr->size_in_bytes);
        l2_block_start = map_ptr->p_base & L3_PT_ADDRESS_BITS; 
        l2_block_end = l2_block_start + 0x200000;     
        if( end_address > l2_block_end )
        {
...
)
{
 ...
 , BL_ERR_NULL_PTR_PASSED|BL_ERROR_GROUP_BOOT );
  ...
  /* Convert our base address into an L1 page table index */
  address_base_index = (memory_address & L1_PT_INDEX) >> GB_TO_BYTES_SHIFT; 
