...
{
   BOOLEAN tuning_failed = FALSE; 
   SDCC_STATUS     rc         = SDCC_NO_ERROR;
   sdcc_cmd_type   sdcc_cmd;
   sdcc_switch_data_type switch_status;
   enum sdhci_uhs_mode_t speed_mode = SDHCI_SDR12_MODE; 
...
SelectMode:
   ...
   if ( ( SDCC_ACCESS_MODE_SDR104 & switch_status.funcgrp_support_func[0] ) && ( FALSE == tuning_failed ) )
   {
      speed_mode = SDHCI_SDR104_MODE; 
      access_mode = SDCC_ACCESS_MODE_SDR104;
   }
   else if ( SDCC_ACCESS_MODE_DDR50 & switch_status.funcgrp_support_func[0] )
   {
      speed_mode = SDHCI_DDR50_MODE; 
      access_mode = SDCC_ACCESS_MODE_DDR50; 
   }
   else if ( SDCC_ACCESS_MODE_SDR50 & switch_status.funcgrp_support_func[0] )
   {
      speed_mode = SDHCI_SDR50_MODE; 
      access_mode = SDCC_ACCESS_MODE_SDR50;
   }
   else if ( SDCC_ACCESS_MODE_HS & switch_status.funcgrp_support_func[0] )
   {
      speed_mode = SDHCI_SDR25_MODE; 
   ...
   sdcc_cmd.cmd_arg = SDCC_SET_MODE | SDCC_QUERY_SUPPORTED_FUNCS | speed_mode;
   sdcc_cmd.flags = SDCC_TRANSFER_FLAG_IS_READ_TRANSFER;
   sdcc_pdev->mem.block_mode = SDCC_MEM_BYTE_MODE;
   ...
   if ( ( 0 == switch_status.max_curr_consumption ) ||
        ( speed_mode != switch_status.funcgrp_status[0] ) )
   {
   ...
   if ( SDCC_NO_ERROR == rc )
   {
      /* For SDR104, execute tuning. If tuning fails, disable DLL and
         change timing mode to SDR50. Then, go back and select the
         next best speed mode. */
      if ( SDHCI_SDR104_MODE == speed_mode )
      {
         ...
         if ( SDCC_NO_ERROR != rc )
         {
            ...
            sdcc_pdev->speed_mode = SDCC_DATA_TRANSFER_MODE; 
            ...
            rc = sdcc_hc_set_uhs_mode_select( sdcc_pdev, SDHCI_SDR50_MODE );
            if ( SDCC_NO_ERROR != rc )
            {
               return rc; 
            }
