...
/**
 * Register SDCC_HC_REG_DLL_CONFIG_2 
 * Change MCLK_FREQ_CALC for DLL configuration
 *
 * @param nControllerId
 *   SD controller number, as described in \ref nControllerId .
 *  
 ...
 */
inline void
HAL_sdhci_DLL2_ClockConfig (const UINT32 nControllerId, UINT8 mclk_freq_calc)
{
   UINT32 val = HWIO_SDC_REG_IN(HC_REG_DLL_CONFIG_2, (UINTN)sdcc_reg_hc[nControllerId]);
 ...
 */
inline void
HAL_sdhci_DllConfig3(const UINT32 nControllerId)
{
   ...
   {
      HWIO_SDC_REG_OUT (HC_REG_DLL_CONFIG_3, (UINTN)sdcc_reg_hc[nControllerId], 0x10);
   }
}
 ...
 */
inline void
HAL_sdhci_DllUserCtl (const UINT32 nControllerId)
{
   UINT32 controller_ver = HWIO_HC_REG_FC_FE_HC_VENDOR_VERSION_NUM_BMSK & HWIO_SDC_REG_IN(HC_REG_FC_FE, (UINTN)sdcc_reg_hc[nControllerId]); 

   if (controller_ver >= 0x72000000)
   {
      HWIO_SDC_REG_OUT (HC_REG_DLL_USER_CTL, (UINTN)sdcc_reg_hc[nControllerId], 0x2C010800);
   }
}
