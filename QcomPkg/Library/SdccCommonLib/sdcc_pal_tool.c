...
SDCC_CARD_TYPE sdcc_handle_find_card( struct sdcc_device *handle )
{
   ...
   else if (sdcc_bsp_get_slot_type(driveno) & SDCC_BSP_SD_SLOT)
   {
      ...
      /* Check whether PMIC operations are allowed for UHS-1 cards by enabling the VDD
         that can be switched. If not successful, then card_type is set to SDHC instead */
