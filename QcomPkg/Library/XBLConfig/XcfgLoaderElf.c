...
Copyright 2016 - 2018 by Qualcomm Technologies, Inc.  All Rights Reserved.
=============================================================================*/
/*===========================================================================
...
----------   ---     ----------------------------------------------------------
04/06/18     vk       Allocate memory for hashes
...
#define SIZE_SHA512_HASH_BYTES (64)
#define MAX_HASH_COUNT         (40)
#define MI_HEADER_SIZE         (288)
#define PAGE_SIZE              (0x1000)
#define PAGE_ROUND_UP(x) ( ((x) + PAGE_SIZE-1)  & (~(PAGE_SIZE-1)) ) 

    total_hash_size = PAGE_ROUND_UP( (SIZE_SHA512_HASH_BYTES*MAX_HASH_COUNT) + MI_HEADER_SIZE );

    status = xcfg_malloc(total_hash_size, (xcfg_elf_hash_buffer));
    if(status != XBLCONFIG_SUCCESS)
    {
      break;
    }
    ...
    {
      qmemcpy((*xcfg_elf_hash_buffer), hash_buffer_ptr, total_hash_size);
    }
    
    xcfg_elf_info_ptr->hash_buffer_size = total_hash_size;
