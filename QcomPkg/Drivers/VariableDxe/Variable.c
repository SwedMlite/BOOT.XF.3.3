/** @file Variable.c
  ...
  Copyright (c) 2011-2015,2017-2018 Qualcomm Technologies, Inc.  All 
...
STATIC UINT32 gVarStorePartitionId = 0;
  ...
  )
{
  ...
  /* TABLE_ID_RESERVED */
  Status = SetTableInfo(&VarInitParams->TableInfoArray[i++], gVarStorePartitionId,
    ...
    )
{
  ...
  if (EFI_ERROR (Status))
  {
    DEBUG((EFI_D_ERROR, "Varstore partition %g - %r\n", &VarStorePartitionGuid, Status));
    goto Exit;
  }
  else if (MaxHandles != 1)
  {
    DEBUG((EFI_D_ERROR, "Varstore partition %g - #handles = %d\n", &VarStorePartitionGuid, MaxHandles));
...
SanitizeVarStores (VOID)
{
  ...
  Status = GetConfigValue("VarStorePartitionId", &gVarStorePartitionId);
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_INFO, "Failed to read VarStorePartitionId from uefiplat.cfg!\n"));

    if (boot_from_emmc())
      gVarStorePartitionId = QSEE_STOR_EMMC_USER;
    else if (boot_from_ufs())
      gVarStorePartitionId = QSEE_STOR_EMMC_GPP1;
    else if (boot_from_spi_nor())
      gVarStorePartitionId = QSEE_STOR_SPINOR_ALL;
    else
    {
      DEBUG((EFI_D_ERROR, "Invalid VarStorePartitionId!\n"));
      return Status;
    }
  }

  /* Assume that all VarStores are on the same physical partition.
     This can be later split per table if required - support is in place */ 
  switch(gVarStorePartitionId)
  {
    /* For RPMB and SPINOR usecases, defer to secure world */
  ...
  /* UEFI_BS_NS */
  Status = CheckVarStorePartitionExists(&HandleFilter, &gVarStoreBSNSPartitionGuid);
  /* UEFI_RT_NS */
  Status |= CheckVarStorePartitionExists(&HandleFilter, &gVarStoreRTNSPartitionGuid);
  /* UEFI_RT_S */
  Status |= CheckVarStorePartitionExists(&HandleFilter, &gVarStoreRTSPartitionGuid);
  if (EFI_ERROR (Status))
    DEBUG((EFI_D_WARN, "VarStore NS-world partition not found\n"));

  /* Ignore varstore partition not found failure, we will automatically attempt RPMB from S-world */
  Status = EFI_SUCCESS;
  ...
  )
{
  ...
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR, "Failed to start UefiSecApp TA\n"));
    return Status;
  }

  /* Confirm VarStore locations exist */
  Status = SanitizeVarStores();
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR, "Failed to find varstore partitions\n"));
    return Status;
  }
