...
)
{
  ...
  if (pPhysEntry->pVirtAddr != 0 && !pCtxt->bAlwaysMap)
  {
  ...
  if (pVirtEntry->dwRefCount == 0)
  {
    ...
    /*
     * If the virtual address is already assigned in the base map, use that
     */
    if (pPhysEntry->pVirtAddr != 0)
    {
      nVirtAddr = HWIO_PAGE_BASE(pPhysEntry->pVirtAddr);
    }
    else
    {
