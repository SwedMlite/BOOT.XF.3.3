...
static boolean is_cbw_valid(IN USBMSD_DEV *UsbMsd)
{
  ...
  DBG(EFI_D_ERROR, "wrong param, bCBWCBLength %d, cbw_len %d, dCBWSignature %d, bcbwlun %d",
    cbw->bCBWCBLength, cbw_len, cbw->dCBWSignature, cbw->bCBWLUN);
...
DEV_STATE handle_new_cbw(IN USBMSD_DEV *UsbMsd)
{
  ...
  if (is_cbw_valid(UsbMsd))
  {
    ...
    DBG(EFI_D_ERROR, "cbw_valid_failure");
