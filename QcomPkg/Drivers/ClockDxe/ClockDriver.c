...
/*
  ...
  $Header: //components/rel/boot.xf/3.2/QcomPkg/Drivers/ClockDxe/ClockDriver.c#10 $
  $DateTime: 2019/10/29 10:13:56 $
  $Author: pwbldsvc $
 ...
 */
#define CLOCK_CONFIG_DELAY_ATTEMPTS 300    /* Expressed in iterations */
...
)
{
  ...
  eResult = Clock_GetPropertyHandle("ClockLogDefaults", &LogHandle);


  if (eResult == DAL_SUCCESS)
  {
    ULogFront_RealTimeInit (&pDrvCtxt->hClockLog, "Clock Log",
                            ((ClockLogType *)LogHandle)->nLogSize,
                            ULOG_MEMORY_LOCAL, ULOG_LOCK_OS);

    pDrvCtxt->pBSP->nFlags |= ((ClockLogType *)LogHandle)->nGlobalLogFlags;
  }
...
)
{
  ...
  /*-----------------------------------------------------------------------*/
  /* Clear out existing BSP volatile data.                                 */
  /*-----------------------------------------------------------------------*/

  for (nIndex = 0; nIndex < pBSP->nNumRails; nIndex++)
  {
    for (i = 0; i < RAIL_VOLTAGE_LEVEL_NUM_LEVELS; i++)
    {
      pBSP->aRails[nIndex]->VRegConfig.aCornerRefCounts[i].eCorner = RAIL_VOLTAGE_LEVEL_OFF;
      pBSP->aRails[nIndex]->VRegConfig.aCornerRefCounts[i].nRefCount = 0;
    }
    pBSP->aRails[nIndex]->VRegConfig.eMinLevel = RAIL_VOLTAGE_LEVEL_OFF;
    pBSP->aRails[nIndex]->VRegConfig.NPAHandle = NULL;
    for (i = 0; i < RAIL_VOLTAGE_LEVEL_NUM_LEVELS; i++)
    {
      pBSP->aRails[nIndex]->VRegConfigSuppressible.aCornerRefCounts[i].eCorner = RAIL_VOLTAGE_LEVEL_OFF;
      pBSP->aRails[nIndex]->VRegConfigSuppressible.aCornerRefCounts[i].nRefCount = 0;
    }
    pBSP->aRails[nIndex]->VRegConfigSuppressible.eMinLevel = RAIL_VOLTAGE_LEVEL_OFF;
    pBSP->aRails[nIndex]->VRegConfigSuppressible.NPAHandle = NULL;
    pBSP->aRails[nIndex]->bSuspendVoltageRequest = FALSE;
  }

  for (nIndex = 0; nIndex < pBSP->nNumClocks; nIndex++)
  {
    pBSP->aClocks[nIndex].nReferenceCount = 0;
  }

  for (nIndex = 0; nIndex < pBSP->nNumDomains; nIndex++)
  {
    ClockDomainNodeType *pDomain = pBSP->aDomains[nIndex];

    pDomain->nReferenceCount = 0;
    pDomain->nReferenceCountSuppressible = 0;
    pDomain->VRegRequest.eVRegLevel = RAIL_VOLTAGE_LEVEL_OFF;
    pDomain->VRegRequest.bSuppressible = FALSE;
  ...
  {
    pBSP->aSources[nIndex]->nReferenceCount = 0;
    pBSP->aSources[nIndex]->nReferenceCountSuppressible = 0;
    pBSP->aSources[nIndex]->nReferenceCountExternal = 0;
    pBSP->aSources[nIndex]->VRegRequest.eVRegLevel = RAIL_VOLTAGE_LEVEL_OFF;
    pBSP->aSources[nIndex]->VRegRequest.bSuppressible = FALSE;
    pBSP->aSources[nIndex]->pActiveFreqConfig = NULL;
    pBSP->aSources[nIndex]->NPAHandle = NULL;
    pBSP->aSources[nIndex]->NPAHandleSuppressible = NULL;
    pBSP->aSources[nIndex]->nCalibration = 0;
    pBSP->aSources[nIndex]->nFlags &= ~CLOCK_FLAG_SOURCE_CALIBRATED;
  }

  for (nIndex = 0; nIndex < pBSP->nNumPowerDomains; nIndex++)
  {
    pBSP->aPowerDomains[nIndex].nReferenceCount = 0;
    pBSP->aPowerDomains[nIndex].nReferenceCountHW = 0;
    pBSP->aPowerDomains[nIndex].VRegRequest.eVRegLevel = RAIL_VOLTAGE_LEVEL_OFF;
    pBSP->aPowerDomains[nIndex].VRegRequest.bSuppressible = FALSE;
  }
  ...
  memset((void *)pDrvCtxt, 0x0, sizeof(ClockDrvCtxt));

  /*-----------------------------------------------------------------------*/
  /* Restore the BSP data pointer.                                         */
  /*-----------------------------------------------------------------------*/
...
)
{
  ...
  Clock_VoltageRequest(pDrvCtxt, pClockDomain->pRail,
...
)
{
  ...
  if (!bSuppressibleRequest)
  {
    ...
    if (pClockDomain->nReferenceCount == 0)
    {
      ...
      /* ActiveMuxConfig could be NULL if there is no BSP data
       * or the domain is at safe configuration,we want to ensure
       * the rail is at least ON to enable the clock
       */
...
)
{
  ...
  if (pClock->nReferenceCount == 0)
  {
    ...
    if (!CLOCK_FLAG_IS_SET(pClock, IGNORE_ON_STATUS) &&
    ...
    {
      eResult = Clock_GetSourceId(pDrvCtxt, CLOCK_XO_NAME, &nXoId);
      ...
      {
        ULOG_RT_PRINTF_1 (pDrvCtxt->hClockLog,
                          "DALSYS_LOGEVENT_FATAL_ERROR: Unable to resolve source ID for the %s source name",
                          CLOCK_XO_NAME);
...
)
{
    ...
    if (pNewConfig != pDomain->pActiveMuxConfig)
    {
        ...
        QDSS_SWEVENT(CLOCK_EVENT_CLOCK_FREQ, pClock->HALClock.nDebugMuxSel, nResultFreq * nMult);
...
)
{
  ...
  {
    return Clock_GetClockNumFreqLevels(pDrvCtxt, pClock, pnData);
  }
...
)
{
  ...
  /*
   * SPECIAL CASE PROCESSING: If the source is specified as -1 (all 'F's), then
   * the client is requesting a voltage update only and no other processing.
   */
  if (nSource == -1)
  {
    /*
...
)
{
  ...
  if (bFound)
  {
    *pnPowerDomainId = CLOCK_MAKE_ID(CLOCK_ID_CATEGORY_POWERDOMAIN, nPowerDomainIndex);
    return DAL_SUCCESS;
  }
  else
  {
...
)
{
  ...
  if (pPowerDomain->nReferenceCount == 0)
  {
    ...
    if (pPowerDomain->pRail)
    {
      ...
      eResult = Clock_VoltageRequest(pDrvCtxt, pPowerDomain->pRail,
...
)
{
      ...
      eConfig == CLOCK_CONFIG_MARK_INSUPPRESSIBLE)
  {
     ...
     */
    if (Clock_CheckNameList("ClockSuppressibleList", pClock->szName) == FALSE)
    {
      ...
      eConfig == CLOCK_CONFIG_RESUME_REF_COUNTING)
  {
     ...
     * Make sure this action is allowed for this clock
     */
    if (Clock_CheckNameList("ClockRefCountSuspendedList", pClock->szName) == FALSE)
    {
      return DAL_ERROR_NOT_ALLOWED;
    }
  }
      ...
      eConfig == CLOCK_CONFIG_FOOTSWITCH_PERIPHERAL_ALLOW_ON)
  {
     ...
     * Make sure this action is allowed for this clock
     */
    if (Clock_CheckNameList("ClockForceMemCoreAndPeriphList", pClock->szName) == FALSE)
    {
      return DAL_ERROR_NOT_ALLOWED;
    }
  }
      ...
      eConfig == CLOCK_CONFIG_BRANCH_ONLY_RAW_DISABLE)
  {
     ...
     * Make sure this action is allowed for this clock
     */
    if (Clock_CheckNameList("ClockBranchOnlyRawControlList", pClock->szName) == FALSE)
    {
      return DAL_ERROR_NOT_ALLOWED;
    }
  }
...
)
{
  ...
  if (hClient->type == NPA_CLIENT_INITIALIZE)
  {
     ...
     */
    eResult = Clock_EnableClock(pDrvCtxt, pNPACtxt->pClock);
    if (eResult != DAL_SUCCESS)
    {
      ULOG_RT_PRINTF_0 (pDrvCtxt->hClockLog,
...
/**
  ...
  @param pClock    [in]  Pointer to the clock.
...
/* =========================================================================
**  Function : Clock_GetClockDomainId
** =========================================================================*/
/**
  Looks up a clock domain ID for a specified clock.

  This function takes the ID of a clock and returns the corresponding clock
  domain ID.

  @param pDrvCtxt        [in]  Pointer to the clock driver context.
  @param pClock          [in]  Pointer to the clock.
  @param pnClockDomainId [out] Pointer to the clock domain ID to be filled in.
...
DALResult Clock_GetClockDomainId
(
  ClockDrvCtxt      *pDrvCtxt,
  ClockNodeType     *pClock,
  ClockDomainIdType *pnClockDomainId
)
{
  ClockBSPType *pBSP;
  uint32        i;
  ...
  *pnClockDomainId = CLOCK_ID_INVALID;

  /*-----------------------------------------------------------------------*/
  /* Find the Clock Domain ID to return.                                   */
  /*-----------------------------------------------------------------------*/

  for (i = 0; i < pBSP->nNumDomains; i++)
  {
    if (pBSP->aDomains[i] == pClock->pDomain)
    {
      *pnClockDomainId = CLOCK_MAKE_ID(CLOCK_ID_CATEGORY_CLOCK_DOMAIN, i);
    }
  }

  /*-----------------------------------------------------------------------*/
  /* Validate the Clock Domain ID before returning.                        */
  /*-----------------------------------------------------------------------*/

  if (*pnClockDomainId == CLOCK_ID_INVALID)
  {
    return DAL_ERROR_NOT_FOUND;
  }
...
)
{
  ...
  eResult = Clock_GetPropertyHandle(szListName, &NameListHandle);

  if (eResult == DAL_SUCCESS)
  {
    ClockNameList = (ClockNameListType *)NameListHandle;
  }
  else
  {
    return FALSE;
