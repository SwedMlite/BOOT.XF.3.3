   ...
   OUT UINTN              *ClockDividerId)
{
  ...
  if (ClockDividerId == NULL)
  {
    return EFI_INVALID_PARAMETER;
  }

  eResult = DalClock_GetDividerId(hClock, ClockDividerName, (ClockDividerIdType *)ClockDividerId);
   ...
   IN  UINT32                    Divider)
{
  ...
  eResult = DalClock_SetDivider(hClock,
                                     (ClockIdType)ClockId,
                                     (uint32)Divider);
  ...
  OUT UINT32              *pnFrequencyLevels)
{
  ...
  if (pnFrequencyLevels == NULL)
  {
    return EFI_INVALID_PARAMETER;
  }

  eResult = DalClock_GetCPUFrequencyLevels(hClock, nCPU, pnFrequencyLevels);
  ...
  IN  UINT32                     nSize)
{
  ...
  if (pCPUFreqPlan==NULL)
  {
    return EFI_INVALID_PARAMETER;
  }

  eResult = DalClock_GetCPUFrequencyPlan(hClock, nCPU, (ClockFreqPlanType*)pCPUFreqPlan, nSize);
  ...
  OUT  UINT32               *pnResultFreqHz)
{
  ...
  eResult = DalClock_SetCPUFrequency(hClock, nCPU, nFrequencyHz, pnResultFreqHz);
  ...
  OUT UINT32               *pnFrequencyHz)
{
  ...
  eResult = DalClock_GetCPUFrequency(hClock, nCPU, pnFrequencyHz);
