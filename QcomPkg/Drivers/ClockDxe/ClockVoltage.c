...
)
{
  ...
  if (eMinLevel != pVRegConfig->eMinLevel)
  {
    if (pVRegConfig->NPAHandle == NULL)
    {
      if (eMinLevel > pRail->eBootCorner)
      {
        ULOG_RT_PRINTF_3 (pDrvCtxt->hClockLog,
          "DALSYS_LOGEVENT_FATAL_ERROR: Voltage request greater than boot default before %s is available (0x%x > 0x%x).",
          pRail->szName, eMinLevel, pRail->eBootCorner);
        ERR_FATAL("Clock_UpdateVoltageRequests", 0, 0, 0);
      }
    }
    else if (!pRail->bSuspendVoltageRequest)
    {
      ...
      clock_npa_issue_scalar_request(pVRegConfig->NPAHandle, eMinLevel);
...
)
{
  ...
  Clock_UpdateVoltageRequests(pDrvCtxt, pRail, pCurrentVRegConfig, RAIL_VOLTAGE_LEVEL_OFF);
...
)
{
  ...
  {
    clock_npa_issue_scalar_request(
      pRail->VRegConfig.NPAHandle,
...
)
{
  ...
  for (i = 0; i < pDrvCtxt->pBSP->nNumRails; i++)
  {
    ...
    eResult = Clock_GetBootCorner(pRail->szName, &pRail->eBootCorner);
    if (eResult != DAL_SUCCESS)
    {
      ULOG_RT_PRINTF_1 (pDrvCtxt->hClockLog,
        "DALSYS_LOGEVENT_FATAL_ERROR: Unable to determine default boot voltage for %s.", pRail->szName);
      ERR_FATAL("Clock_UpdateVoltageRequests", 0, 0, 0);
      return DAL_ERROR;
    }
