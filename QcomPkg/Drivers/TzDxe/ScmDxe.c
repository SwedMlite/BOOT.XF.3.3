...
)
{
  ...
  if ((GetConfigValue("SecurityFlag", &configFlag) != EFI_SUCCESS) || ((configFlag & 0x100) != 0x100)) 
  {
    DEBUG((EFI_D_INFO, "Failed to get QseeLogEnableFlag \n"));
  }
  else 
  {
    if (QseeRegisterLogBuffer() != EFI_SUCCESS)
    {
      DEBUG ((EFI_D_WARN, "QseeRegisterLogBuffer: Failed to register log buffer message \n"));
    }
  }
