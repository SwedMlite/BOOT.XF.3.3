...
/*=============================================================================
 ...
 11/22/17   nk       Return success TzeLoaderProtocolInit in error case for pre-sil
...
{
  ShmBridgeProtocol->ShmBridgeFree(ShmBridgeProtocol, Buffer);
  return;
}
...
/* 
* Load embedded services (cmnlib and cmnlib64) using QSEE scm command
*/
EFI_STATUS
QseeLoadEmbeddedServicesSyscall()
{
   EFI_STATUS              Status;
...
)
{
  ...
  {
    // Attempt to load from SPI-NOR
    Status =  LoadFileFromPartition(Attrib, Path, &gEfiSpiNor0Guid, pPartitionGuid, NULL, &CfgBuffPtr,
                                    &ReadFileSize);
    if (Status != EFI_SUCCESS)
    {
      Status = LoadFileFromPartition(Attrib, Path, &gEfiNvme0Guid, pPartitionGuid, NULL, &CfgBuffPtr,
                                     &ReadFileSize);
