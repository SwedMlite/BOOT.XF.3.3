/** @file QseeComDxe.c
  ...
  Copyright (c) 2012-2018 Copyright Qualcomm Technologies, Inc. 
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 11/08/18   jc     Enable loading of TZ-embedded common libs
...
)
{
  ...
  CHAR8*                  TzAppsLabel = "TZApps Reserved";
  ...
  {        
    ...
    {
      DEBUG((EFI_D_INFO, "LoadSecureApps: Unable to find TZApps mem region addr\n"));
  ...
  if (version >= computed_version)
  {
    ...
    {
      /*Check if this TZ supports embedded libs and if the libs are actually loaded*/
	  Status = QseeLoadEmbeddedServicesSyscall();
	  if(Status == EFI_SUCCESS)
	  {
	    DEBUG ((EFI_D_WARN, "Embedded Images Supported and Commonlibs loaded\n"));
	  }
	  else
	  {
	    DEBUG ((EFI_D_WARN, "Embedded Commonlib Images not loaded Status:%d\r\n", Status));
  ...
  else
  {
    ...
    if(EFI_ERROR(Status))
    {
      ...
      if(Status != EFI_SUCCESS)
      {
        DEBUG ((EFI_D_ERROR, "Unable to load Keymaster using label Status:%d\r\n", Status));
        ASSERT_EFI_ERROR(Status);
      }
      //gRT->ResetSystem(EfiResetCold, EFI_SUCCESS, 0, NULL);
    }
    DEBUG ((EFI_D_INFO, "LoadSecureApps: Load app from partition(%s): Status = 0x%p, AppId = %d\r\n",
