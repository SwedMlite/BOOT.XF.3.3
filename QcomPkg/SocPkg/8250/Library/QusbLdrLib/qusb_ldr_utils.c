...
#include "pm_schg_misc.h"         //charger detection api
...
#define QUSB_DCI_PMI8998_INDEX                    (1)
...
// Force EDL check using D+ GND support
 ...
 */
// ===========================================================================
static pm_schg_usb_apsd_result_status_type qusb_fedl_get_chgr_type(void)
{
  ...
  if (PM_ERR_FLAG__SUCCESS != pm_get_pmic_info(QUSB_DCI_PMI8998_INDEX, &pmic_device_info))
  {    
  ...
  while (--charger_det_retry_cnt)
  {
    ...
    pm_err = pm_schg_usb_get_typec_status(QUSB_DCI_PMI8998_INDEX, &typec_status);
    ...
    pm_err = pm_schg_usb_get_apsd_result_status(QUSB_DCI_PMI8998_INDEX, &usb_charger_type);
    if (PM_ERR_FLAG__SUCCESS != pm_err)
    {
  ...
  /* Test Code for HVDCP -- Enable to checking HVDCP only - currently PMIC disabled HVDCP
  if (PM_SCHG_USB_APSD_RESULT_STATUS_DCP_CHARGER == usb_charger_type)
  {
    qusb_dci_delay_ms(2000);
    pm_err = pm_schg_usb_get_apsd_result_status(QUSB_DCI_PMI8998_INDEX, &usb_charger_type);
    if (PM_ERR_FLAG__SUCCESS == pm_err)
    {
      qusb_uart_w_str_log("usb: chgr", qusb_fedl_get_chgr_str(usb_charger_type));
...
boolean qusb_ldr_utils_fedl_check(void)
{
  ...
  qusb_dci_eud_update_serial_number(qusb_dci_get_chip_serial_number());
  ...
  usb_charger_type = qusb_fedl_get_chgr_type();

  if (PM_SCHG_USB_APSD_RESULT_STATUS_CDP_CHARGER == usb_charger_type)
  {
    // continue to execute to comply with CDP requiring D+ high to be asserted within 1sec
    // Logic will assert D+ high 50ms then lower it for CDP
  }
  else if (PM_SCHG_USB_APSD_RESULT_STATUS_SDP_CHARGER != usb_charger_type)
  {
    // if not CDP or SDP, then return FALSE.
    return FALSE; 
  }
  else if (FALSE == qusb_forced_download_feature_supported)
  {
  ...
  if (PM_SCHG_USB_APSD_RESULT_STATUS_CDP_CHARGER == usb_charger_type)
  {
    ...
    qusb_dci_dp_toggle(QUSB_LDR_UTILS_CDP_DP_TOGGLE_MS);
