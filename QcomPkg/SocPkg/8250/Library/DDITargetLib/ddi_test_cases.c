...
{
  return (uint64*)ddr_shared_data->ddr_size_info.ddr_cs0_remapped_addr[0];
}
...
{
  uint64* ret = (uint64*)ddr_shared_data->ddr_size_info.ddr_cs1_remapped_addr[0];
  if (ret == 0) {
...
ddi_error ddr_cmppattern(uint64* start, uint64* end, uint64* pattern, uint64 pattern_size, ddi_cmp_params* cmp_params)
{
  ...
  for (i = 0; i < size; i++) {
    ...
    uint64 expected_val = pattern[i % pattern_size]; // may cause unaligned access
...
void ddi_run_command_delay(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  ...
  *input_iter += sizeof(ddi_delay_params);
  ...
  ((ddi_output_header*)ddi_int_out)->dram_type = ddr_shared_data->device_params.device_type;
...
//#endif
  ...
  #if DDI_PRINT_ENABLE
  snprintf(ddi_log_string, sizeof(ddi_log_string), "start_addr: 0x%lx, end_addr 0x%lx", start, end);
  boot_log_message(ddi_log_string);
  snprintf(ddi_log_string, sizeof(ddi_log_string), "repeat_count: %ld", repeat_count);
  boot_log_message(ddi_log_string);
  #endif
...
void ddi_run_command_wr(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  ...
  *input_iter += pattern_size*sizeof(uint64);
  ...
  boot_log_message(ddi_log_string);
  snprintf(ddi_log_string, sizeof(ddi_log_string), "repeat_count: %ld, random: %d", repeat_count, random);
  boot_log_message(ddi_log_string);
  #endif
  ret |= get_start_end((uint64*)&start, (uint64*)&end);
...
boolean ddi_handle_fail_rdwr(ddi_handle_fail_params* fail_params, uint64* addr, uint64 expected, uint64 read, uint64* output_iter)
{
  ...
  boot_log_message(ddi_log_string);
  snprintf(ddi_log_string, sizeof(ddi_log_string), "  A:0x%016lx%016lx", (uint64)addr>>32, (uint64)addr&0xFFFFFFFF);
  boot_log_message(ddi_log_string);
  snprintf(ddi_log_string, sizeof(ddi_log_string), "  E:0x%016lx%016lx", (uint64)expected>>32, (uint64)expected&0xFFFFFFFF);
  boot_log_message(ddi_log_string);
  snprintf(ddi_log_string, sizeof(ddi_log_string), "  D:0x%016lx%016lx", (uint64)read>>32, (uint64)read&0xFFFFFFFF);
  boot_log_message(ddi_log_string);
  #endif
  ...
  status=save_ddi_data2ddr_partition(ddi_int_out, sizeof(ddi_fail_addr), *output_iter);
  if(status)
...
void ddi_run_command_rdwr(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  ...
  *input_iter += pattern_size*sizeof(uint64);
  ...
  snprintf(ddi_log_string, sizeof(ddi_log_string), "repeat_count: %ld", repeat_count);
  boot_log_message(ddi_log_string);
  snprintf(ddi_log_string, sizeof(ddi_log_string), "verify: %d, random: %d", verify, random);
  boot_log_message(ddi_log_string);
  #endif
...
//  #endif
  ...
  status=save_ddi_data2ddr_partition((uint8*)&output_header, sizeof(ddi_output_header), *output_iter);
  if(status)
  {
    *output_iter += sizeof(ddi_output_header);
    status=save_ddi_data2ddr_partition((uint8*)&rdwr_output, sizeof(ddi_rdwr_output), *output_iter);
    if(status)
    {
      *output_iter += sizeof(ddi_rdwr_output);
    }
  }
...
void ddi_read_ecdt_parameters(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  ...
  *output_iter += sizeof(ddi_output_header);
  ...
  do{
    ...
    number_of_freq = ddr_shared_data->num_ddr_levels-1;
    status=save_ddi_data2ddr_partition((uint8*)&number_of_freq, sizeof(number_of_freq), *output_iter);
    if(status)
    *output_iter += sizeof(number_of_freq);
    ...
    for (freq_plan_index = 1;freq_plan_index < (number_of_freq+1); freq_plan_index++)
    {
      ...
      ecdt_output->number_settings = (uint8)param_cnt/8;
#ifdef DDI_DEBUG_PRINT_ENABLE
...
#endif //DDI_DEBUG_PRINT_ENABLE
      save_ddi_data2ddr_partition((uint8*)buffer, sizeof(ddi_out_interface_settings)- sizeof(ecdt_output->settings_buf) + param_cnt, *output_iter);
...
void ddi_get_ac_timing(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  int8 freq_plan_index;
  uint8 number_of_freq;
  void* buffer = NULL;
  ...
  do{
    ...
    *output_iter += sizeof(number_of_freq);
    ...
    for (freq_plan_index = 1;freq_plan_index < (number_of_freq+1); freq_plan_index++)
    {
      ...
      ac_w_clk_out->number_settings = param_cnt/8;
#ifdef DDI_DEBUG_PRINT_ENABLE
...
#endif //DDI_DEBUG_PRINT_ENABLE
      save_ddi_data2ddr_partition((uint8*)buffer, sizeof(ddi_out_ac_timing_w_clk)- sizeof(ac_w_clk_out->settings_buf) + param_cnt, *output_iter);
...
*   @sa None
*/
void ddi_get_safe_mode_info(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  ddi_output_header output_header;
  void* buffer = NULL;
  size_t buffer_size = 1024;
...
//#if DDI_PRINT_ENABLE
//    snprintf(ddi_log_string, sizeof(ddi_log_string), "safe_mode_ptr %d, setting_buf %d", safe_mode_out, (void*)&(safe_mode_out->settings_buf));
//    boot_log_message(ddi_log_string);
//#endif
  ...
  safe_mode_out->number_settings = param_cnt/8;
  ...
  exit:
    ...
    *output_iter += sizeof(ddi_output_header);
    if (output_header.test_status==DDI_CMD_COMPLETE)
    {
      save_ddi_data2ddr_partition((uint8*)safe_mode_out, sizeof(ddi_out_safe_mode)-sizeof((safe_mode_out->settings_buf))+param_cnt, *output_iter);
...
#if 0
/* ============================================================================
**  Function : ddi_get_ac_timing_wo_clk
** ============================================================================
*/
/*!
*   @brief
*   This function is called to do get AC timing wo clk
*
*   @param cmd_id      -   [IN] ID of the command. Used for logging to flash.
*   @param params      -   [IN] pointer to test case parameters
*   @param input_iter  -   [IN] pointer to current DCB offset
...
void ddi_get_ac_timing_wo_clk(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  ddi_output_header output_header;
  void* buffer = NULL;
  size_t buffer_size = 1024;
  int param_cnt;
  ddi_out_ac_timing_wo_clk* ac_wo_clk_out;

  boot_log_message("DDI: get ac timing independent of clk");
  ...
  ac_wo_clk_out = (ddi_out_ac_timing_wo_clk *)buffer;

  param_cnt = HAL_DDR_FW_eCDT_Get_AC_Timing_Values_without_Clock_Index((void*)&(ac_wo_clk_out->settings_buf), buffer_size-sizeof(ddi_out_ac_timing_wo_clk));
  ...
  ac_wo_clk_out->number_settings = param_cnt/8;


#ifdef DDI_DEBUG_PRINT_ENABLE
  snprintf(ddi_log_string, sizeof(ddi_log_string), "n settings %d ", ac_wo_clk_out->number_settings);
  boot_log_message(ddi_log_string);

  int i;
  uint32* temp = (uint32*)&(ac_wo_clk_out->settings_buf);
  for (i=0;i<(param_cnt/4);i+=2)
  {
    snprintf(ddi_log_string, sizeof(ddi_log_string), "setting %d = 0x%X ", *(temp+i), *(temp+i+1));
    boot_log_message(ddi_log_string);
  ...
  exit:
    ...
    *output_iter += sizeof(ddi_output_header);
    ...
    if (output_header.test_status==DDI_CMD_COMPLETE)
    {
      save_ddi_data2ddr_partition((uint8*)ac_wo_clk_out, sizeof(ddi_out_ac_timing_wo_clk)- sizeof(ac_wo_clk_out->settings_buf)+param_cnt, *output_iter);
      *output_iter += sizeof(ddi_out_ac_timing_wo_clk- sizeof(ac_wo_clk_out->settings_buf)+param_cnt);
    }
    if (buffer != NULL)
    {
      ddi_free(buffer);
...
*   @sa None
*/
void ddi_get_device_info(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  uint8 i;
  ddi_output_header output_header;
  ...
  uint32* reg_value;
  #if DDI_PRINT_ENABLE
  snprintf(ddi_log_string, sizeof(ddi_log_string), "DDI: get device info");
  boot_log_message(ddi_log_string);
  #endif

  reg_value = (uint32*)BOOT_LOGGER_JTAG_ID_FUSE_ADDRESS;
  dev_info.chip_id = *reg_value;
  reg_value = (uint32*)BOOT_LOGGER_SERIAL_NUM_FUSE_ADDRESS;
  dev_info.chip_sn = *reg_value;
  ...
  *output_iter += sizeof(ddi_output_header);
  ...
  save_ddi_data2ddr_partition((uint8*)&dev_info, sizeof(ddi_device_info), *output_iter);
...
void ddi_run_command_empty(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  ddi_output_header output_header;

  boot_log_message("DDI: not supported command");
  output_header.cmd_id = cmd_id;
