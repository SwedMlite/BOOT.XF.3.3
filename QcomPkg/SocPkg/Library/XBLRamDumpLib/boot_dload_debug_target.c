...
$Header: //components/rel/boot.xf/3.2/QcomPkg/SocPkg/Library/XBLRamDumpLib/boot_dload_debug_target.c#6 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/23/19   aus     remove internal heap 
...
{
  boolean add_region_status = FALSE;
  static uint32 aoss_cc_fsm_status_register __attribute__((section (".bss.BOOT_UNCACHED_ZI_ZONE")));
  static uint32 aoss_cc_fsm_ctrl_register __attribute__((section (".bss.BOOT_UNCACHED_ZI_ZONE")));
...
*/
  ...
  //dload_add_binning_info(); --//will remove if not needed during bring-up
