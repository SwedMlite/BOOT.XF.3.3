...
/*=============================================================================
 ...
 11/15/17   ts       Added PcieConfigLibConfigureSMMU prototype
 09/11/17   ts       Added support for SDM1000
 05/22/17   ts       Updated prototype for PcieConfigLibUpdatePortInfo
 04/21/17   ts       Replaced POLL_UNTIL and REG_WRITE_CONDITIONAL mascros with
                     respective functions, as that is very easy to debug and
                     takes up less instruction in compiled version of the code
 10/07/16   ts       Created new API to Enable Clocks with a variable length
                     array of string as input
 10/07/16   ts       Changed POLL_UNTIL macro to use while loop instead of
                     a do while, so as to check for the condition even before
                     wasting with a unit of delay
 08/10/16   ts       Created new APIs to power down PHY and update AML variable
 07/28/16   ts       Included support to configure I/O port
 07/08/16   ts       Include new function to update the global port info
                     strcture based on the result of link training
 07/08/16   ts       code refactoring
 06/08/16   ts       read the device control 2 register based on cap walk &
                     merged the PERST and link training in one function
 02/08/16   ts       Added pcie xml config support
 06/15/15   ts       removed PCIe_ReadNumberofRP, since it is a static now.
                     Included support for pcie capability walk.
 05/13/15   ts       Unified code for multi platforms
 03/18/15   ts       Fixed Copyrights header
 03/16/15   ts       Included GCC BCR register variable
