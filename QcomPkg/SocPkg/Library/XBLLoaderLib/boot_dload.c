...
/*=============================================================================
  ...
  $Header: //components/rel/boot.xf/3.2/QcomPkg/SocPkg/Library/XBLLoaderLib/boot_dload.c#8 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
10/12/18   aus     Move the loader logs to Shared IMEM HLOS region before going to EDL mode
04/19/18   vk      Add log for FDL timeout reset
...
void boot_dload_transition_pbl_forced_dload( void )
{
  ...
  boot_log_message ("Enter emrgency download");
...
)
{
  ...
  */
  ...
  {
     boot_log_message ("EDL: boot_dload_check : boot_qusb_ldr_utils_fedl_check");
     boot_dload_transition_pbl_forced_dload();
  }
...
* 
*/
void boot_dload_handle_forced_dload_timeout( bl_shared_data_type *bl_shared_data )
{

  if(boot_dload_is_forced_dload_timeout_set())
  {
    boot_log_message ("EMERGENCY_DLOAD_TIMEOUT_COOKIE_SET: Reset");
    boot_busywait (300000); /* Wait for log 300 mS */
