...
Copyright 2012-2014,2016,2017,2019 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
============================================================================*/
...
void boot_move_hash_segment_buffer( void )
{
  ...
  {
     status = boot_DALSYS_Malloc(SIZE_24KB,(void **)&boot_hash_segment_imem_ptr);
...
{
  DALResult status = DAL_SUCCESS;
  status = boot_DALSYS_Free((void *)boot_hash_segment_imem_ptr);
  BL_VERIFY(status==DAL_SUCCESS,(uint16)status|BL_ERROR_GROUP_DAL);
