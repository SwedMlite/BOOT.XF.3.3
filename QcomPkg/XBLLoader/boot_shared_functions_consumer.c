...
--------   ---     ------------------------------------------------------------
06/10/19   nm      Add Clock APIs needed for QUPv3 Buses
01/22/19   yps     relocate shared function table
07/06/18   vk       Add return codes for heap functions
...
static boot_shared_functions_type * shared_functions_table = NULL;
...
boolean boot_shared_functions_init(void * shared_function_table)
{
  ...
  shared_functions_table = (boot_shared_functions_type *)shared_function_table;
...
{
  return shared_functions_table->boot_log_get_time();
}

uint32 boot_log_delta_time(uint32 start, char * message)
{
  return shared_functions_table->boot_log_delta_time(start, message);
}
...
{
  return shared_functions_table-> 
    boot_pm_app_pon_debug_hard_reset(); 		
}
...
int boot_DALSYS_HeapInit(void *pHeapBase, uint32 dwHeapSz, uint32 dwInitType) 
{
  return shared_functions_table->boot_DALSYS_HeapInit(pHeapBase,dwHeapSz,dwInitType);
}

int boot_DALSYS_HeapDeInit(void *pHeapBase) 
{
...
pm_err_flag_type pm_lcdb_set_output_volt(uint32 device_index, int32 ldo_mv, int32 ncp_mv, boolean symmetry_ok){
  return shared_functions_table->pm_lcdb_set_output_volt(device_index, ldo_mv, ncp_mv, symmetry_ok);
}

pm_err_flag_type pm_lcdb_config_enable_ctrl(uint32 device_index, pm_lcdb_enable_ctl_type ctl_type, boolean enable)
{
  return shared_functions_table->pm_lcdb_config_enable_ctrl(device_index, ctl_type, enable);
}

pm_err_flag_type pm_lcdb_vreg_ok(uint32 device_index, boolean *vreg_ok)
{
  return shared_functions_table->pm_lcdb_vreg_ok(device_index, vreg_ok);
}
...
{
  return shared_functions_table->pm_ldo_sw_enable(pmic_chip, ldo_peripheral_index, on_off, wait_for_settle);
}
...
{
  return shared_functions_table->pm_smps_sw_enable(pmic_chip, smps_peripheral_index, on_off, wait_for_settle);
}
...
{
  return shared_functions_table->pm_ldo_volt_level(pmic_chip, ldo_peripheral_index, volt_level, wait_for_settle);
}
...
{
  return shared_functions_table->pm_smps_volt_level(pmic_chip, smps_peripheral_index, volt_level, wait_for_settle);
}
...
{
  return shared_functions_table->pm_wled_set_led_duty_cycle(pmic_device_index, modulator, data);
}
...
{
  return shared_functions_table->pm_wled_enable_modulator(pmic_device_index, modulator, enable);
}
...
{
  return shared_functions_table->pm_wled_modulator_src_sel(pmic_device_index, modulator, src);
}
...
{
  return shared_functions_table->boot_PlatformInfo_IsFusion();
}
