...
Copyright 2009 - 2019 by Qualcomm Technologies, Inc.  All Rights Reserved.
=============================================================================*/
  ...
  */
  if((MAX_BOOTLOG_SECONDARY_ENTRIES + 1) >= error_handler_entry_count)
  {
    if(error_handler_entry_count == 1) /* Entering Error Handler for first time */
    {
      ...
      boot_log_message("^^^^- Printing Call Stack -^^^^");
      fp =  boot_crash_dump_data.regs[29];
	  uint64 i = 0;
      while((fp!=0 && i < MAX_CALLSTACK_PRINT)&&(fp < (uint64)Image$$SBL1_STACK_BASE$$Pointer)&&(fp > (uint64)Image$$SBL1_STACK$$Base)){
      i++;
      __asm__ __volatile__("ldr %0, [%1,#8]":"=r"(lr):"r"(fp)); 
      __asm__ __volatile__("ldr %0, [%1]":"=r"(fp):"r"(fp));
      
      qsnprintf(func_name, 26 ,"func_addr  :   %08X", lr);
      boot_log_message(func_name);
      }

      boot_log_message("^^^^^- Done Printing -^^^^^");   
    ...
    {
      boot_log_message("boot_error_handler: Flashless boot. Going for hard reset");
    ...
    else if(!ramdump_allowed)
    {
      ...
      {
        sbl_crashdump_reset_with_dload();        
      }
#else
      boot_log_message("EDL: boot_error_handler : rd not allowed");
    ...
    else
    {
      ...
      of sbl_dload_entry()
      */
      boot_log_message("boot_error_handler: Ramdump allowed. Trying to enter DLOAD");
      sbl_dload_entry();
    }
  }
...
{  uint64 temp;
  ...
  __asm__ __volatile__("mov %0, X0":"=r"(temp));
  boot_crash_dump_data.regs[0] = temp;

  __asm__ __volatile__("mov %0, X1":"=r"(temp));
  boot_crash_dump_data.regs[1] = temp;
  
  __asm__ __volatile__("mov %0, X2":"=r"(temp));
  boot_crash_dump_data.regs[2] = temp;
 
  __asm__ __volatile__("mov %0, X3":"=r"(temp));
  boot_crash_dump_data.regs[3] = temp;

  __asm__ __volatile__("mov %0, X4":"=r"(temp));
  boot_crash_dump_data.regs[4] = temp;

  __asm__ __volatile__("mov %0, X5":"=r"(temp));
  boot_crash_dump_data.regs[5] = temp;
  
  __asm__ __volatile__("mov %0, X6":"=r"(temp));
  boot_crash_dump_data.regs[6] = temp;

  __asm__ __volatile__("mov %0, X7":"=r"(temp));
  boot_crash_dump_data.regs[7] = temp;

  __asm__ __volatile__("mov %0, X8":"=r"(temp));
  boot_crash_dump_data.regs[8] = temp;
  
  __asm__ __volatile__("mov %0, X9":"=r"(temp));
  boot_crash_dump_data.regs[9] = temp;

  __asm__ __volatile__("mov %0, X10":"=r"(temp));
  boot_crash_dump_data.regs[10] = temp;

  __asm__ __volatile__("mov %0, X11":"=r"(temp));
  boot_crash_dump_data.regs[11] = temp;
  
  __asm__ __volatile__("mov %0, X12":"=r"(temp));
  boot_crash_dump_data.regs[12] = temp;

  __asm__ __volatile__("mov %0, X13":"=r"(temp));
  boot_crash_dump_data.regs[13] = temp;

  __asm__ __volatile__("mov %0, X14":"=r"(temp));
  boot_crash_dump_data.regs[14] = temp;
  
  __asm__ __volatile__("mov %0, X15":"=r"(temp));
  boot_crash_dump_data.regs[15] = temp;

  __asm__ __volatile__("mov %0, X16":"=r"(temp));
  boot_crash_dump_data.regs[16] = temp;

  __asm__ __volatile__("mov %0, X17":"=r"(temp));
  boot_crash_dump_data.regs[17] = temp;
  
  __asm__ __volatile__("mov %0, X18":"=r"(temp));
  boot_crash_dump_data.regs[18] = temp;

  __asm__ __volatile__("mov %0, X19":"=r"(temp));
  boot_crash_dump_data.regs[19] = temp;

  __asm__ __volatile__("mov %0, X20":"=r"(temp));
  boot_crash_dump_data.regs[20] = temp;
  
  __asm__ __volatile__("mov %0, X21":"=r"(temp));
  boot_crash_dump_data.regs[21] = temp;

  __asm__ __volatile__("mov %0, X22":"=r"(temp));
  boot_crash_dump_data.regs[22] = temp;

  __asm__ __volatile__("mov %0, X23":"=r"(temp));
  boot_crash_dump_data.regs[23] = temp;

  __asm__ __volatile__("mov %0, X24":"=r"(temp));
  boot_crash_dump_data.regs[24] = temp;
  
  __asm__ __volatile__("mov %0, X25":"=r"(temp));
  boot_crash_dump_data.regs[25] = temp;

  __asm__ __volatile__("mov %0, X26":"=r"(temp));
  boot_crash_dump_data.regs[26] = temp;

  __asm__ __volatile__("mov %0, X27":"=r"(temp));
  boot_crash_dump_data.regs[27] = temp;

  __asm__ __volatile__("mov %0, X28":"=r"(temp));
  boot_crash_dump_data.regs[28] = temp;

  __asm__ __volatile__("mov %0, X29":"=r"(temp));
  boot_crash_dump_data.regs[29] = temp;

  __asm__ __volatile__("mov %0, X30":"=r"(temp));
  boot_crash_dump_data.regs[30] = temp;
  
  temp = (uint64)&sbl_save_regs;
  boot_crash_dump_data.pc = temp;

  __asm__ __volatile__("mov %0, SP":"=r"(temp));
  boot_crash_dump_data.sp_el1 = temp;
  
  __asm__ __volatile__("mrs %0, ACTLR_EL1":"=r"(temp));
  boot_crash_dump_data.actlr_el1 = temp;

  __asm__ __volatile__("mrs %0, CCSIDR_EL1":"=r"(temp));
  boot_crash_dump_data.ccsidr_el1 = temp;
  
  __asm__ __volatile__("mrs %0, CLIDR_EL1":"=r"(temp));
  boot_crash_dump_data.clidr_el1 = temp;

  __asm__ __volatile__("mrs %0, CNTKCTL_EL1":"=r"(temp));
  boot_crash_dump_data.cntkctl_el1 = temp;
  
  __asm__ __volatile__("mrs %0, CPACR_EL1":"=r"(temp));
  boot_crash_dump_data.cpacr_el1 = temp;

  __asm__ __volatile__("mrs %0, CSSELR_EL1":"=r"(temp));
  boot_crash_dump_data.csselr_el1 = temp;
  
  __asm__ __volatile__("mrs %0, ELR_EL1":"=r"(temp));
  boot_crash_dump_data.elr_el1 = temp;

  __asm__ __volatile__("mrs %0, ESR_EL1":"=r"(temp));
  boot_crash_dump_data.esr_el1 = temp;
  
  __asm__ __volatile__("mrs %0, FAR_EL1":"=r"(temp));
  boot_crash_dump_data.far_el1 = temp;
  
  __asm__ __volatile__("mrs %0, MAIR_EL1":"=r"(temp));
  boot_crash_dump_data.mair_el1 = temp;
  
  __asm__ __volatile__("mrs %0, MIDR_EL1":"=r"(temp));
  boot_crash_dump_data.midr_el1 = temp;
  
  __asm__ __volatile__("mrs %0, MPIDR_EL1":"=r"(temp));
  boot_crash_dump_data.mpidr_el1 = temp;
  
  __asm__ __volatile__("mrs %0, SCTLR_EL1":"=r"(temp));
  boot_crash_dump_data.sctlr_el1 = temp;
  
  __asm__ __volatile__("mrs %0, SPSR_EL1":"=r"(temp));
  boot_crash_dump_data.spsr_el1 = temp;
  
  __asm__ __volatile__("mrs %0, TCR_EL1":"=r"(temp));
  boot_crash_dump_data.tcr_el1 = temp;
  
  __asm__ __volatile__("mrs %0, TPIDR_EL1":"=r"(temp));
  boot_crash_dump_data.tpidr_el1 = temp;
  
  __asm__ __volatile__("mrs %0, TTBR0_EL1":"=r"(temp));
  boot_crash_dump_data.ttbr0_el1 = temp;
  
  __asm__ __volatile__("mrs %0, TTBR1_EL1":"=r"(temp));
  boot_crash_dump_data.ttbr1_el1 = temp;
  
  __asm__ __volatile__("mrs %0, VBAR_EL1":"=r"(temp));
  boot_crash_dump_data.vbar_el1 = temp;
}
...
void boot_query_xpu_violation( void )
{
  ...
  char * xpu_violation_msg = "XPU Violation Occurred\n";
