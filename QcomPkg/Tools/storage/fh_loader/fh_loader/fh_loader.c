 ...
 *
 * Release Version: 19.06.10.18.44 (Jun 10 2019 @ 18:44)
 *
...
=============================================================================
 ...
 $Header: //components/rel/boot.xf/3.2/QcomPkg/Tools/storage/fh_loader/fh_loader/fh_loader.c#2 $
...
./fh_loader --port=/dev/ttyUSB0 --sendimage=big.bin --verify_programming
 ...
 *
 */
void string_replace(char *in, size_t len, char *find, char *new)
{
  ...
  while (ahead <= len)
  {
    ...
    {
      memscpy (tail, len - ahead, new, new_len);
      ahead += find_len;
      behind += new_len;
    }
    else
...
void DumpFFURawProgram(struct FFUImage_t * FFU)
{
    ...
    for (i = 0; i < FFUStoreHeader->dwWriteDescriptorCount; i++) 
    {
        ...
        {
            BlockDataEntriesPtr = (struct FFUBlockDataEntry_t *)((char *)BlockDataEntriesPtr + (sizeof(struct FFUBlockDataEntry_t)+(BlockDataEntriesPtr->dwLocationCount - 1)*sizeof(struct FFUDiskLocation_t)));
        }
        else 
        {
            BlockDataEntriesPtr = (struct FFUBlockDataEntry_t *)((char *)BlockDataEntriesPtr + sizeof(struct FFUBlockDataEntry_t));
        }
        dwBlockOffset++;
    }
