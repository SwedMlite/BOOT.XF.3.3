/** @file CrashDumpFlagApp.c
  ...
  Copyright 2013, 2015-2016, 2018-2019 Qualcomm Technologies, Inc.  All Rights Reserved.
...
STATIC UINT8 EnableOnstorageDump = 0xFF;
...
PrintOfflineCrashDumpValuesApp ( VOID )
{
  ...
  /* Print EnableOnstorageDump */
  Size = sizeof(Value);
  Status = gRT->GetVariable(ENABLE_ONSTORAGE_DUMP_VARNAME,
                            &gQcomTokenSpaceGuid,
                            NULL,
                            &Size,
                            &Value);
  if (Status != EFI_SUCCESS)
    AsciiPrint("  Could not retrieve UEFI variable: EnableOnstorageDump, Status = 0x%08x\r\n", Status);
  else
    AsciiPrint("  EnableOnstorageDump: 0x%02x\r\n", Value);
...
ProcessValues( VOID )
{
  ...
  if (EnableOnstorageDump != 0xFF)
  {
    Status = gRT->SetVariable(ENABLE_ONSTORAGE_DUMP_VARNAME,
                              &gQcomTokenSpaceGuid,
                              EFI_VARIABLE_BOOTSERVICE_ACCESS |
                              EFI_VARIABLE_NON_VOLATILE,
                              sizeof(EnableOnstorageDump),
                              &EnableOnstorageDump);
    if (Status != EFI_SUCCESS)
    {
      AsciiPrint("ERROR: Failed to set EnableOnstorageDump UEFI variable\r\n");
      return Status;
    }

    AsciiPrint("Set EnableOnstorageDump: 0x%02x\r\n", EnableOnstorageDump);
  }
...
DeleteValue( CHAR8* ValueStr )
{
  ...
  else if (AsciiStriCmp ((CHAR8*)ValueStr, "EnableOnstorageDump") == 0)
  {
    Status = gRT->SetVariable(ENABLE_ONSTORAGE_DUMP_VARNAME,
                              &gQcomTokenSpaceGuid,
                              EFI_VARIABLE_BOOTSERVICE_ACCESS |
...
ProcessCommand (UINTN Argc, CHAR8** Argv)
{
  ...
  while (i < Argc)
  {
    ...
    else if (AsciiStriCmp ((CHAR8*)CmdStr, "-EnableOnstorageDump") == 0)
    {
      EnableOnstorageDump = (UINT8)(AsciiStrHexToUintn(ArgStr) & 0xFF);
    }
...
PrintUsage ( VOID )
{
  ...
  AsciiPrint ("Note: -del EnableOnstorageDump will delete the EnableOnstorageDump UEFI variable\r\n");
