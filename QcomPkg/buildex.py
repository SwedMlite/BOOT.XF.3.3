...
#
# Copyright 2017, 2018, 2019 by Qualcomm Technologies, Inc.
#                 All Rights Reserved.
...
def main():
  ...
  if options.alias :
    Alias_config =  remove_json_comments(os.path.join(os.environ["WORKSPACE"],
  ...
  Tool_chain_json_config = remove_json_comments(os.path.join(os.environ["WORKSPACE"],
  ...
  for target_path in target_paths:
    ...
    while (index < len(variants) ) or (index == 0):
      ...
      # check if atleast one of the variant or common json is present
      if var_json_path == "" and json_path == "":
        sys.exit("Atleast one of the common or variant buildconfig json file should be present")
      
      # if variant buildconfig is found then process it to object
      if var_json_path != "":
        build_dirt_var = pre_parser_json_file(var_json_path)
        # find the image which we should process for the variant
        try:
          var_images = match_images(options.image,build_dirt_var)
        except Exception as error:
          print error
          sys.exit(MATCH_IMAGES_ERROR_CODE)
      
      # if both buildconfig json are present
      if len(build_dirt) and len(build_dirt_var):
        # add variant json components and images along with the process to original build_dirt
        for k in build_dirt.keys():
          if k in build_dirt_var.keys():
            build_dirt[k].extend(build_dirt_var[k])
        
        # add images from variant json to images list
        images.extend(var_images)          
      
        # check if same image or components are present after merging variant json
        for k in build_dirt.keys():
          dict_list =[]
          for dict_v in build_dirt[k]:
...
def component_dependency_check(up_level_dependency, dependencies):
  ...
  cur_dep_pkg_buildconfigdirt = json.loads(cur_dep_pkg_buildconfig)
  for component in cur_dep_pkg_buildconfigdirt['Components']:
...
# process_components
#####################################################################
def process_components(options,variant_flavor,process_dirt,build_dirt,tools_dirt,merge_dict):
  dll_paths=[]
  for component in process_dirt['Components']:

    dsc_path=process_component(options,variant_flavor,component,process_dirt,build_dirt,tools_dirt)
    # Determine what to merge
    if dsc_path:
      dll_paths.append(re.sub('[()]', '',generate_merge_item(merge_dict,dsc_path,variant_flavor,process_dirt)))
  return dll_paths

#####################################################################
# build component
#####################################################################
def process_component(options,variant_flavor,component,process_dirt,build_dirt,tools_dirt):
  component_dirt = None
  build_cmd = []
  dependencies = {}
  dsc_path = None
  ...
  if "Dependency" in component_dirt:
    dependencies={ARGUMENTS["$TARGETPKG"],component_dirt['Dependency']['Pkg']} 
   # for dependency incomponent_dirt['Dependency']: 
    component_dependency_check(component_dirt['Dependency'],dependencies)
    if not os.path.exists(replace_arguments(component_dirt['Dependency']['ImagePath'])):
      needs_compile = True 
    else:
      needs_compile = False
  if needs_compile:
    for i in range(len(component_dirt['Process'])):
      ...
      if build_cmd[0] == "build":
        ...
        if ((options.components and component_dirt['Name'].encode('ascii').upper() in options.components.upper()) or options.components == "") and not check_components_status(build_cmd):
...
def setup_environment():
  ...
  if(os.path.isdir(os.path.join(os.environ["WORKSPACE"], QCOM_DIR,"Tools","sectools"))):
    os.environ["SECTOOLS_DIR"]=os.path.join(os.environ["WORKSPACE"], QCOM_DIR,"Tools","sectools")
    print"sectools present in boot build"
  else:	
    if "SECTOOLS_DIR" not in os.environ:
      print 'Error: Sectools environment variable not present'
      if platform.system() is "Windows":
        os.environ["SECTOOLS_DIR"] = "\\\\swayam\\sectools\\public\\5.27"
      else:
        os.environ["SECTOOLS_DIR"] = "/prj/sectools/public/5.27"
  ...
  os.environ["SECTOOLS_CFG_PATH"] = os.path.join(os.environ["SECTOOLS_DIR"], "config","integration")
  os.environ["SECTOOLS_SCRIPTS_PATH"] = os.environ["SECTOOLS_DIR"]
