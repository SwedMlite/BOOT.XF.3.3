...
Copyright 2013-2015, 2017-2018 by Qualcomm Technologies, Inc.  All Rights Reserved.
============================================================================*/
...
--------   ---         --------------------------------------------------
11/14/19   rakhi       Added alignment in boot_raw_partition_dump_section_header
07/11/18   yps         Added Raw dump debugging log
...
struct boot_raw_partition_dump_section_header ALIGN(4)
...
static boot_boolean boot_process_raw_ram_dump_sections()
{
 ...
 /* Iterate through mem_debug_info table and process each section*/
  for(; i < ram_dump_sections_num ; i++)
  {
    ...
    curr_sec_size = dload_mem_debug_mem_length(i);
    curr_sec_base_addr = dload_mem_debug_mem_base(i);
    DL_LOG("RawDump Free space:0x%llx, Dump start address:0x%llx, size 0x%llx",free_space_size,curr_sec_base_addr,curr_sec_size);
     ...
     dload_mem_debug_supported())
  {
    ...
    if(trans_if != NULL)
    {
      ...
      {        
        ...
        */
        DL_LOG("RawDump successfully, Reset the device");
