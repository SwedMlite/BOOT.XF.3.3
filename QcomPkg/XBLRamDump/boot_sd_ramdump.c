...
Copyright 2010-2014, 2017-2019 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
============================================================================*/
...
--------   ---         --------------------------------------------------
10/08/18   vk          Do not open multiple files
07/11/18   yps         Added Ram dump log
...
static void boot_ramdump_reset( void )
{
  ...
  if(!(fs_handle < 0))
  {
    ...
    {
      boot_hw_reset(BOOT_APQ_COLD_DEBUG_RESET);
    }
    else{
...
boot_update_ramdump_header_file (int index)
{
    ...
    baseaddr = dload_mem_debug_mem_base(index);
    length = dload_mem_debug_mem_length(index);
...
static void boot_process_sd_dumps( void )
{
  ...
  uint32 memregion_index, str_size;
  ...
  for(memregion_index = 0; memregion_index < max_region_entries; memregion_index++)
  {
    ...
    DL_LOG("start addr:0x%016llx size:0x%016llx file: %s",baseaddr,length,filename);
    ...
    if(INVALID_FILE_HANDLE != fs_handle)
    {
      ...
      if(boot_sd_rd_errdata.err_no != EFS_NO_ERR)
      {
        ...
        boot_update_errfile("errfile.txt", filename);
      }
      else
      {
        DL_LOG("done");
