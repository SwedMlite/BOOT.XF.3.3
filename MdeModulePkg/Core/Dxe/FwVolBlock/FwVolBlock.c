  ...
  )
{
  ...
  //
  //  For memory mapped case we shouldn't really need to cache, since
  //   accessing from cached memory would be of exactly same speed if its
  //   not, there is not tangible benefit to caching at the expense of more
  //   memory usage. But to keep the usage to minimum, increase the Block
  //   size to higher value in fdf's so that total blocks is smaller
  //   TODO: optimize later to avoid even the block map
